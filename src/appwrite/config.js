...
export class Service {
  ...
  async deletePost(slug) {
    try {
      // Here we are not returning because it is not needed, though if we still return it nothing wrong in it
      await this.databases.deleteDocument(
        conf.appwriteDatabaseId,
        conf.appwriteCollectionId,
        slug
      );
      return true; // Msg for frontend
    } catch (error) {
      console.log("Appwrite error::deletePost::error", error);
      return false; // Msg for frontend
    }
  }
